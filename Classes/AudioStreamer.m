         ...
         if ([self retrieveShoutcastMetaData]) {
            ...
            if (parsedHeaders)
            {
               ...
               for (int i=streamStart; i < length; i++)
               {
                  ...
                  {
                     NSString *tmp = [[NSString alloc] initWithBytes: bytes+i length: metaDataBytesRemaining encoding: NSUTF8StringEncoding];
                     [metaDataString appendString: [tmp autorelease]];

                     [self updateMetaData:metaDataString];
                     dataBytesRead = 0;

                     i += (metaDataBytesRemaining - 1);
